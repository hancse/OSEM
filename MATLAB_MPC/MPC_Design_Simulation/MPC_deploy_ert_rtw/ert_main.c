...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(MPC_deploy_M) == (NULL));
  while (runModel) {
    ...
    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(MPC_deploy_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(MPC_deploy_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  mwRaspiInit();
  MW_launchPyserver();
  rtmSetErrorStatus(MPC_deploy_M, 0);

  /* Initialize model */
  MPC_deploy_initialize();
